...
.pull-right[
```{r, echo=FALSE,out.width="450px",  out.height="400px",fig.cap="",fig.show='hold',fig.align='center'}
...
A inferência estatística é composta por um conjunto de resultados estatísticos/matemáticos baseados na **Teoria das Probabilidades** que podem ser utilizadas para realizar três coisas que normalmente são de interesse na estatística:
...
1. Implementar novas metodologias na área de teoria da informação; </br></br>
2. Desenvolver novas metodologias na área de teoria da informação que poderão ser utilizadas como teste de adequação de ajuste de uma distribuições $F$ à um conjunto de dados. A ideia é tentar conseguir formas de obter boas estatíticas de adequação de ajuste que funcionem bem em pequenas amostras; </br></br>
3. Melhorar o desempenho computacional da biblioteca usando **C/C++** usando a biblioteca **Rcpp**.
...
Um  procedimento comumente usado é o Teste da Razão de Verossimilhanças (TRV)  que consiste  em aceitar ou rejeitar a hipótese nula $H_0$ com base em um conjunto de observações (amostra). O TRV faz uso de uma estatística $\lambda$ definida por

$$\lambda = \frac{L(\theta_0|\pmb x)}{L(\pmb\theta|\pmb x)},$$
em que $L(\cdot)$ é uma função que denominamos de função de verossimilhança.
</br>
...
**Problema**: quando o tamanho da amostra é pequeno, o TRV pode levar a inferências imprecisas.
